//
//  XDContinuousBootCrashPreventTool.m
...
@implementation XDContinuousBootCrashPreventTool

+ (BOOL)launchContinuousCrashProtect
{
    NSAssert(repairBlock, @"repairBlock is nil!");
    if (Logger) Logger(@"XDContinuousBootCrashPreventTool: Launch continuous crash report");
    [self setIsFixing:NO];
    
    NSInteger launchCrashes = [self crashCount];
    // 上报
    if (launchCrashes >= kContinuousCrashOnLaunchNeedToReport) {
        if (Logger) Logger([NSString stringWithFormat:@"XDContinuousBootCrashPreventTool: App has continuously crashed for %@ times. Now synchronize uploading crash report and begin fixing procedure.", @(launchCrashes)]);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kCrashOnLaunchTimeIntervalThreshold * NSEC_PER_SEC)), dispatch_get_main_queue(), ^(void){
        // APP活过了阈值时间，重置崩溃计数
        if (Logger) Logger([NSString stringWithFormat:@"XDContinuousBootCrashPreventTool: long live the app ( more than %@ seconds ), now reset crash counts", @(kCrashOnLaunchTimeIntervalThreshold)]);
...
{
    if ([[self class] crashCount] >= kContinuousCrashOnLaunchNeedToFix) {
        return NO;
    }
