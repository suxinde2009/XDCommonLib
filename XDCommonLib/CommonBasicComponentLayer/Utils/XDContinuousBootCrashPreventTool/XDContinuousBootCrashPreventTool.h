//
//  XDContinuousBootCrashPreventTool.h
 ...
 */
@interface XDContinuousBootCrashPreventTool : NSObject

/**
 * 启动连续 crash 保护方法。
 * 前置条件：在 App 启动时注册 crash 处理函数，在 crash 时调用[XDContinuousBootCrashPreventTool addCrashCountIfNeeded]。
