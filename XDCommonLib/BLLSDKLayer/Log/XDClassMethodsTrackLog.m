...
#pragma mark - XDClassMethodsTrackLogBlock

@interface XDClassMethodsTrackLogBlock : NSObject
...
@implementation XDClassMethodsTrackLogBlock
...
@interface XDClassMethodsTrackLog()

@property (strong, nonatomic) NSMutableDictionary *blockCache;

+ (instancetype)sharedInstance;

- (XDClassMethodsTrackLogBlock *)blockWithTarget:(id)target;
...
NSDictionary *qhd_canHandleTypeDic() {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dict = @{[NSString stringWithUTF8String:@encode(char)] : @"(char)",
...
NSInvocation *qhd_callOrigionMethod(id target, SEL sel, va_list vas, void **returnValue, BOOL hasReturn, NSString **desc) {
    ...
    *desc = methodDesc;
    ...
    XDClassMethodsTrackLogBlock *block = [[XDClassMethodsTrackLog sharedInstance] blockWithTarget:target];
...
@implementation XDClassMethodsTrackLog
...
+ (void)logMethodWithClass:(Class)aClass
    ...
    if (aClass) {
        XDClassMethodsTrackLogBlock *block = [[XDClassMethodsTrackLogBlock alloc] init];
        block.targetClassName = NSStringFromClass(aClass);
        block.condition = condition;
        block.before = before;
        block.after = after;
        [[XDClassMethodsTrackLog sharedInstance].blockCache setObject:block forKey:block.targetClassName];
    }
...
+ (instancetype)sharedInstance {
    static XDClassMethodsTrackLog *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[self alloc] init];
        _sharedInstance.blockCache = [NSMutableDictionary dictionary];
    });
...
- (XDClassMethodsTrackLogBlock *)blockWithTarget:(id)target {
    ...
    XDClassMethodsTrackLogBlock *block = [self.blockCache objectForKey:NSStringFromClass(class)];
    while (block == nil) {
